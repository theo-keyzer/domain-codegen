...
#$run  doc3.act,check.act ../nexus.unit,artifact.unit > check.txt
#if [ $? != 0 ]; then echo doc.act unit.unit,artifact.unit,act.unit has errors; fi

$run  doc3.act,check.act unit.unit,artifact.unit,act.unit > gen-doc.txt
if [ $? != 0 ]; then echo doc.act unit.unit,artifact.unit,act.unit has errors; fi

$run  doc3.act,check.act tsu.unit,tsu-auto.unit,tsu-ext.unit > tsu-doc.txt
if [ $? != 0 ]; then echo doc.act tsu.unit,tsu-auto.unit,tsu-ext.unit has errors; fi

$run  doc3.act,check.act net5.unit > ml-doc.txt
if [ $? != 0 ]; then echo doc.act net5.unit has errors; fi

$run  doc3.act,check.act nexus.unit > nexus-doc.txt
