...
var (
	rioLexer = lexer.MustSimple([]lexer.SimpleRule{
		{"Comment", `//.*`},
		{"Whitespace", `\s+`},
		// Ident now allows dots, colons, and underscores
...
type ValueDef struct {
	Json *string `( @JsonQuote | @Ident )` // Identifiers also check for JSON
	Raw  *string `| @RawQuote`
}
...
func (f *RioFile) ToLegacyComponents(filename string) []Component {
	...
	for _, c := range f.Components {
		comp := Component{
			Type:       c.Type,
			Filename:   filename, // Include filename in output
		...
		for _, field := range c.Fields {
			var val any
			if field.Value.Raw != nil {
				// Literals (", """, ~, ~~~) - skip leading newline if ~~~
				val = cleanValue(*field.Value.Raw, false, true)
			} else if field.Value.Json != nil {
				// JSON-eligible (', ''', Ident)
				val = cleanValue(*field.Value.Json, true, false)
			}

			switch field.Key {
			case "key":
				comp.Name = fmt.Sprint(val)
			case "parent":
				comp.Parent = fmt.Sprint(val) // Handle parent like key
			default:
				comp.Fields[field.Key] = val
			}
		}
...
func cleanValue(s string, detectJson bool, isRaw bool) any {
	...
	// Skip first newline if it's a ~~~ block
...
func unquote(s string) string {
	...
	if len(s) >= 6 {
		for _, d := range []string{`"""`, `'''`, `~~~`} {
			if s[:3] == d && s[len(s)-3:] == d { return s[3 : len(s)-3] }
		}
	}
	// Single delimiters
	for _, d := range []string{`"`, `'`, `~`} {
		if s[:1] == d && s[len(s)-1:] == d { return s[1 : len(s)-1] }
	}
	return s
}
